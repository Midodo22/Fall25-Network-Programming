...
def build_response(sender, status, message, params=None, **extra):
    """
    Build a response payload while allowing optional metadata such as game_name.
    """
    if params is None:
        params = []
    response = {"sender": sender, "status": status, "message": message, "params": params}
