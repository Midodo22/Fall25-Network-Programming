...
async def process_client_message(message, username, client_reader, client_writer, db_reader, db_writer):
    try:
        ...
        if command == "REGISTER":
            ...
            if username:
                await handle_watch_room(params, username, client_writer)
            else:
                await ut.send_message(client_writer, ut.build_response("lobby", "error", "Not logged in"))
...
async def handle_watch_room(params, username, writer):
    if len(params) != 1:
        await ut.send_message(writer, ut.build_response("lobby", "error", "Invalid WATCH command"))
        return
    room_id = params[0]
    async with tetris_server.rooms_lock:
        room = tetris_server.rooms.get(room_id)
        if not room:
            await ut.send_message(writer, ut.build_response("lobby", "error", "Room does not exist"))
            return
        if room.get("type", "public") != "public":
            await ut.send_message(writer, ut.build_response("lobby", "error", "Cannot watch a private room"))
            return
        if room.get("status") != "In Game":
            await ut.send_message(writer, ut.build_response("lobby", "error", "Room is not currently in game"))
            return
    async with tetris_server.game_servers_lock:
        server_entry = tetris_server.game_servers.get(room_id)
    if not server_entry:
        await ut.send_message(writer, ut.build_response("lobby", "error", "No active game found for this room"))
        return
    message = {
        "status": "watch_info",
        "room_id": room_id,
        "game_host": config.GAME_HOST,
        "game_port": server_entry["port"]
    }
    await ut.send_message(writer, message)
