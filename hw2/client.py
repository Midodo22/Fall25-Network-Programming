...
async def initiate_game(game_in_progress, writer, room_id):
    ...
    try:
        await start_game_session(peer_info.get("game_host"), peer_info.get("game_port"), room_id)
...
async def start_game_session(game_ip, game_port, room_id, max_retries=10, retry_delay=2):
    ...
    for candidate in candidates:
        success = await connect_with_retries(candidate, game_port, max_retries, retry_delay)
        if success:
            return
        else:
...
async def connect_with_retries(host, port, max_retries, retry_delay):
    retries = 0
    while retries < max_retries:
        try:
            ...
            await connect_to_game_server(host, port, username)
            return True
...
async def connect_to_game_server(ip, port, username):
    """
    ...
    try:
        ...
        logging.info(f"Sent JOIN message with username: {username}")
        ...
        # Start game loop
        await game_loop(reader, writer, username)
...
async def game_loop(reader, writer, username):
    if TEXT_MODE_CLIENT or pygame is None:
        return await text_mode_game_loop(reader, writer, username)
    """
    ...
    player_states = {}
    opponent_name = None
    info_banner = "Waiting for another player..."
    final_message = None
        ...
        try:
            while True:
                ...
                if msg_type == "SNAPSHOT":
                    ...
                    info_banner = "Game started! Clear lines to win."
    ...
    while running:
        ...
        if game_active and pygame.key.get_focused():
...
async def text_mode_game_loop(reader, writer, username):
    player_states = {}
    opponent_name = None
    info_banner = "Waiting for another player..."
    final_message = None
    ...
    def render():
        ...
        print("=== Multiplayer Tetris (Text Mode) ===")
        ...
        try:
            while True:
                ...
                if msg_type == "SNAPSHOT":
                    ...
                    info_banner = "Game started! Type commands to move."
                elif msg_type == "GAME_OVER":
                    final_message = msg
                    info_banner = f"Winner: {msg.get('winner')}"
                    game_active = False
